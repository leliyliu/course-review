...
IP的服务模型是尽力而为交付服务，而UDP是不可靠的服务，但是TCP提供了一种可靠数据传输服务。

#### 多路复用和多路分解

一个进程有一个或多个**套接字**，它相当于从网络向进程传递数据和从进程向网络传递数据的门户。

将传输互层报文段中的数据交付到正确的套接字的工作称为**多路分解**

在源主机从不同套接字中收集数据块，并为每个数据块封装上首部信息（这将以后用于分解）从而生成报文段，然后将报文段传递到网络，所有这些工作称为**多路复用**
...
这些特殊字段是**源端口号字段**和**目的端口号字段**，端口号是一个16bits 的值，0~1023被称为**周知端口号**。

##### 关于面向连接和非连接的多路复用和分解

一般而言，可以这样理解，即面向非连接的多路复用和分解是使用UDP协议的，而且一般其由一个二元组决定（目的IP地址+目的端口号），而面向连接的则是TCP协议，是由一个四元组决定的（源IP + 端口 + 。。。。）

通常而言，我们说一个五元组是指： 传输层协议 +源IP端口+ 目的IP端口

当今的高性能Web服务器通常只是用一个进程，但是为每个新的客户连接创建一个具有新连接套接字的新线程。



#### 无连接传输：UDP

传输层最低限度必须提供一种复用分用服务，以便在网络层和正确的应用级进程之间传递数据。

DNS是一个通常使用UDP的应用层协议的例子。

###### ==UDP的原因：很多应用更适合用UDP，原因主要如下==：

1. 关于何时、发送什么数据的应用层控制更为精细。由于TCP 的拥塞控制机制，导致TCP将继续重新发送数据报文段直到目的主机收到此报文段并加以确认，但是有时候，我们不希望过分地延迟报文段的传送，并且能容忍一些数据丢失。
2. 无需连接建立，TCP建立连接要三次握手，而这样的话，DNS会慢很多。
3. 无连接状态
4. 分组首部开销小，TCP报文段有20字节的首部开销，而UDP只有8字节

使用UDP的应用是可以实现可靠数据传输的，这可通过在应用程序自身建立可靠性机制来完成。



UDP报文段结构，主要包括这样几个部分：源端口号，目的端口号，长度，校验和和应用数据，前四个部分每部分长度均为2字节。
...
仅在网络层提供链接服务的计算机网络称为**虚电路网络（VC）**，提供无连接服务的计算机网络称为**数据报网络**

端系统向网络发送指示虚电路启动和终止的报文，以及路由器之间传递的用于建立虚电路的报文，被称为**信令报文**，交换这些报文的协议被称为**信令协议**

#### 数据报网路

在数据报网络中，当由多个匹配时，该路由器使用**最长前缀匹配规则**，即在该表中寻找最长的匹配项。

##### 交换结构

有三种不同的交换模式

**经内存交换**：这是最简单、最早的路由器交换方式，它的操作和I/O设备类似。

**经总线交换**： 通常以这种方式完成：让输入端口为分组预先计划一个交换机内部标签（首部），指示本地输出端口，使分组在总线上传送和传输到输出端口。

**经互联网络交换**：克服单一、共享式总线带宽限制的一种方法。

#### 网际协议

##### ICMP（互联网控制报文协议）

对于数据报格式，其中包括了这样几个部分：版本号，首部长度，服务类型，数据报长度，标识、标志、片偏移，寿命（TTL），协议，首部校验和，源和目的IP地址，选项，数据。

利用MTU进行分片，将一个数据报分为多个片。在进行分片的时候，一个数据报分为多个片，这些片的标识ID是相同的，偏移决定了其具体位置，然后标志为1表示还有片，而标志位0表示这是最后一片。

**CIDR（无类别域间路由选择）**，和在CIDR之前的方式不同，其采用的是一种**分类编址**方式，以8 16 24bit为网络前缀。

#### DHCP

动态主机配置协议，来完成配置主机的IP地址。DHCP是一个客户-服务器协议，客户通常是新到达的主机，它要获得包括自身使用的IP地址在内的网络配置信息。一般来说分为四个步骤：DHCP服务器发现，DHCP服务器提供，DHCP请求和DHCP ACK。
